...
import { setFilterData } from '@/store/slices/allProspectsSlice'
import {
    components,
    ControlProps,
    OptionProps,
...
const CustomControl = ({ children, ...props }: ControlProps<Option>) => {
    const selected: Option | undefined = props.getValue()[0]
    return (
        <Control {...props}>
            {selected && (
...
const AllProspectsTableFilter = () => {
    const dispatch = useAppDispatch()
    const { status } = useAppSelector((state) => state.allProspects.filterData)
...
export default AllProspectsTableFilter
